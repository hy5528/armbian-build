...
if [[ $RELEASE == xenial || $RELEASE == bionic || $RELEASE == focal || $RELEASE == groovy || $RELEASE == jammy ]]; then
		DISTRIBUTION="Ubuntu"
	else
		DISTRIBUTION="Debian"
fi
...
DEBOOTSTRAP_LIST="locales gnupg ifupdown apt-utils apt-transport-https ca-certificates bzip2 console-setup cpio cron \
	...
	wireless-regdb  dmsetup rsync tzdata"
...
case $RELEASE in
	...
	xenial)
		...
		DEBOOTSTRAP_LIST+=" btrfs-tools crda"
	...
	stretch)
		DEBOOTSTRAP_COMPONENTS="main"
		DEBOOTSTRAP_LIST+=" rng-tools crda"
	...
	bionic)
		...
		DEBOOTSTRAP_LIST+=" rng-tools fdisk crda"
	...
	bullseye)
		DEBOOTSTRAP_COMPONENTS="main"
		DEBOOTSTRAP_LIST+=" haveged fdisk crda"
